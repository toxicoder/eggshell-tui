...
if [[ -n "$BASH_VERSION" ]]; then
  if (( BASH_VERSINFO[0] < 4 )); then
    printf "Error: EggshellTUI requires Bash 4.0 or higher.\n" >&2
    exit 1
  fi
elif [[ -n "$ZSH_VERSION" ]]; then
  if (( ${ZSH_VERSION%%.*} < 5 )); then
    printf "Error: EggshellTUI requires Zsh 5.0 or higher.\n" >&2
    exit 1
  fi
fi
...
# Apps must trap cleanup: trap eggshelltui_cleanup EXIT
_EGGSHELLTUI_TEMP_FILES=()
...
eggshelltui_calc_size() {
  ...
  # Minimum constraints
  if (( EGGSHELLTUI_HEIGHT < 5 )); then EGGSHELLTUI_HEIGHT=5; fi
  if (( EGGSHELLTUI_WIDTH < 20 )); then EGGSHELLTUI_WIDTH=20; fi
}

# Cleanup helper
eggshelltui_cleanup() {
  if (( ${#_EGGSHELLTUI_TEMP_FILES[@]} > 0 )); then
    rm -f "${_EGGSHELLTUI_TEMP_FILES[@]}"
  fi
}

# Helper to create tracked temp file
# Usage: eggshelltui_mktemp VAR_NAME
eggshelltui_mktemp() {
  local var_name="$1"
  local temp_file
  temp_file=$(mktemp)
  _EGGSHELLTUI_TEMP_FILES+=("$temp_file")

  if [[ -n "$BASH_VERSION" ]]; then
    printf -v "$var_name" '%s' "$temp_file"
  else
    read -r "$var_name" <<< "$temp_file"
  fi
}
...
# 1. Menu Component
# Usage: eggshelltui_menu "Title" VAR_NAME "Option1" "Desc1" "Option2" "Desc2" ...
# Returns: 0 on selection, 1 on cancel. Selection index stored in VAR_NAME.
eggshelltui_menu() {
  local title="$1"
  local var_name="$2"
  shift 2
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" || "$EGGSHELLTUI_BACKEND" == "whiptail" ]]; then
    ...
    local idx=1
    for ((i=0; i<${#options[@]}; i+=2)); do
      printf "%d) %s - %s\n" "$idx" "${options[i]}" "${options[i+1]}"
      ((idx++))
    done
    read -r -p "Select an option: " choice
    # validate input
    if [[ "$choice" =~ ^[0-9]+$ ]] && (( choice < idx )) && (( choice > 0 )); then
       if [[ -n "$BASH_VERSION" ]]; then
         printf -v "$var_name" '%s' "$choice"
       else
         read -r "$var_name" <<< "$choice"
       fi
       return 0
    fi
    return 1
...
eggshelltui_checklist() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" || "$EGGSHELLTUI_BACKEND" == "whiptail" ]]; then
    ...
    printf "=== %s ===\n" "$title"
    printf "Note: Multi-select not fully supported in text mode fallback yet.\n"
    return 1
  fi
}
...
eggshelltui_radiolist() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" || "$EGGSHELLTUI_BACKEND" == "whiptail" ]]; then
    ...
    printf "Radio list fallback not implemented.\n"
    return 1
  fi
}
...
eggshelltui_password() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" || "$EGGSHELLTUI_BACKEND" == "whiptail" ]]; then
    ...
    if [[ -n "$BASH_VERSION" ]]; then
      local input_val
      read -s -p "Password: " -r input_val
      printf "\n" # Newline after silent read
      printf -v "$var_name" '%s' "$input_val"
    else
...
eggshelltui_form() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" ]]; then
    ...
    if (( form_height > EGGSHELLTUI_HEIGHT )); then form_height=$EGGSHELLTUI_HEIGHT; fi
    ...
    for ((i=0; i<${#fields[@]}; i+=2)); do
      ...
      # Fallback for form must handle cancel properly.
      # Since we are in wizard, cancel means cancel whole form.
      if ! eggshelltui_input "$l ($((i/2+1))/${#fields[@]})" temp_res "$init"; then
        return 1
      fi
    ...
    if [[ -n "$BASH_VERSION" ]]; then
       # Securely copy the array using eval as it's the only way to assign to dynamic array name
       local declare_str
...
eggshelltui_calendar() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" ]]; then
    ...
    local input_date=""
    if eggshelltui_input "Enter Date (YYYY-MM-DD):" input_date "$current_date"; then
       if [[ -n "$BASH_VERSION" ]]; then
...
eggshelltui_timebox() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" ]]; then
    ...
    local input_time=""
    if eggshelltui_input "Enter Time (HH:MM:SS):" input_time "$current_time"; then
       if [[ -n "$BASH_VERSION" ]]; then
...
eggshelltui_range() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" ]]; then
    ...
    # Fallback
    local input_val=""
    if eggshelltui_input "Enter Value ($min - $max):" input_val "$def"; then
       # TODO: Validate range?
       if [[ -n "$BASH_VERSION" ]]; then
         printf -v "$var_name" '%s' "$input_val"
       else
...
eggshelltui_fileselect() {
  ...
  if [[ "$EGGSHELLTUI_BACKEND" == "dialog" ]]; then
    ...
    # Fallback
    local input_val=""
    if eggshelltui_input "Enter File Path:" input_val "$init_path"; then
       if [[ -n "$BASH_VERSION" ]]; then
         printf -v "$var_name" '%s' "$input_val"
       else
         read -r "$var_name" <<< "$input_val"
...
eggshelltui_enter_loop() {
  if [[ -z "$CURRENT_SCREEN" ]]; then
    printf "Error: CURRENT_SCREEN not set.\n" >&2
    exit 1
  fi
